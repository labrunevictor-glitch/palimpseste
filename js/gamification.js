...
function unlockAchievement(id) {
    ...
    renderAchievements();
    // La sync cloud se fait périodiquement via app.js, pas besoin de sync immédiate
}
...
let lastSyncTime = 0;
let lastSyncedHash = null; // Hash des données pour éviter sync inutile
const SYNC_DEBOUNCE = 300000; // 5 minutes entre chaque sync (évite surcharge)
 ...
 */
async function syncProgressWithCloud(force = false) {
    // Vérifier si connecté
    if (!window.supabaseClient || !window.currentUser) {
        return false;
    }
    
    // Debounce pour éviter trop de requêtes (sauf si forcé)
    const now = Date.now();
    if (syncInProgress || (!force && now - lastSyncTime < SYNC_DEBOUNCE)) {
        return false;
    }
    
    // Calculer un hash simple des données pour éviter sync inutile
    const currentHash = JSON.stringify({
        badges: state.achievements?.length || 0,
        reads: state.readCount || 0
    });
    
    // Si rien n'a changé et pas forcé, skip
    ...
    try {
        ...
        // Mémoriser le hash pour éviter resync inutile
 ...
 * Utilisé uniquement au beforeunload ou cas critiques
 */
async function forceSyncToCloud() {
    await syncProgressWithCloud(true); // Force la sync
}
