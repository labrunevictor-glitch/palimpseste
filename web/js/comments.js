...
async function loadComments(extraitId, parentContainer) {
    ...
    try {
        ...
        if (!comments || comments.length === 0) {
            ...
            if (countEl) {
                const displayedCount = parseInt(countEl.textContent) || 0;
                if (displayedCount !== 0) {
                    countEl.textContent = '0';
                    countEl.classList.add('is-zero');
                    const toggleBtn = countEl.closest('.comments-toggle');
                    if (toggleBtn) {
                        toggleBtn.innerHTML = `💬 <span id="commentCount-${extraitId}">0</span> commentaire`;
                    }
                    // Corriger en base si désynchronisé
                    if (supabaseClient) {
                        supabaseClient.from('extraits').update({ comments_count: 0 }).eq('id', extraitId).catch(() => {});
                    }
                }
            }
            return;
        ...
        // Important: utiliser le compte réel, pas la valeur en cache qui peut être désynchronisée
        const realCount = comments.length;
        const countEl = document.getElementById(`commentCount-${extraitId}`);
        if (countEl) {
            const displayedCount = parseInt(countEl.textContent) || 0;
            countEl.textContent = realCount;
            countEl.classList.toggle('is-zero', realCount === 0);
            // Mettre à jour le texte "commentaire(s)" dans le bouton parent si applicable
            const toggleBtn = countEl.closest('.comments-toggle');
            if (toggleBtn) {
                toggleBtn.innerHTML = `💬 <span id="commentCount-${extraitId}">${realCount}</span> commentaire${realCount !== 1 ? 's' : ''}`;
            }
            
            // Si désynchronisation détectée, corriger côté serveur (silencieux)
            if (displayedCount !== realCount && supabaseClient) {
                supabaseClient
                    .from('extraits')
                    .update({ comments_count: realCount })
                    .eq('id', extraitId)
                    .then(() => console.log(`🔄 Compteur commentaires synchronisé: ${displayedCount} → ${realCount}`))
                    .catch(() => {}); // Silencieux si échec (pas de permissions)
            }
        }
...
function renderCommentItem(comment, profileMap, reactionsByComment, myReactionByComment, extraitId) {
    ...
    const editedLabel = comment.edited_at ? `<span class="comment-edited-label" title="${t('modified_on')} ${new Date(comment.edited_at).toLocaleString()}"> • ${t('modified')}</span>` : '';
            ...
            <div class="comment-content">
                ...
                <div class="comment-footer">
                    <div class="comment-footer-actions">
                        ${canEdit ? `<button class="comment-edit" title="${t('tooltip_modify')}" onclick="startEditComment('${comment.id}', '${extraitId}')">✎</button>` : ''}
                        ${canDelete ? `<button class="comment-delete" title="${t('tooltip_delete')}" onclick="deleteComment('${comment.id}', '${extraitId}')">🗑️</button>` : ''}
                        <button class="comment-react-btn" title="${t('tooltip_react')}" onclick="openCommentReactionPicker('${comment.id}', this)">😊</button>
                    </div>
                </div>
            </div>
        </div>
